...
def color_dict(labelFolder, classNum):
    ...
    for i in tqdm(range(len(ImageNameList))):
        ...
        img1 = cv2.imread(ImagePath,flags=2)
        px = np.unique(img1)
        colorDict_GRAY = np.unique(np.append(colorDict_GRAY,px))
        #  如果是灰度，转成RGB
        if (len(img.shape) == 2):
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB).astype(np.uint32)  # img = cv2.imread('1.png', cv2.IMREAD_GRAYSCALE)
        #  为了提取唯一值，将RGB转成一个数
...
rgb,gray = color_dict("datasets\gta5\labels", 50)
print(rgb)
print(gray)