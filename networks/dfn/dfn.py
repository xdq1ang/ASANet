...
from networks.dfn.base_model import resnet101
from networks.dfn.seg_oprs import ConvBnRelu, BNRefine, RefineResidual, \
    ChannelAttention
...
class DFN(nn.Module):
    def __init__(self, out_planes, criterion, aux_criterion, alpha, pretrained_model=None, norm_layer=nn.BatchNorm2d):
        ...
        self.backbone = resnet101(pretrained_model, norm_layer=self.norm_layer,
                                  bn_eps=1e-5,
                                  bn_momentum=0.1,
        ...
        self.global_context = nn.Sequential(
                                        ...
                                        ConvBnRelu(2048, smooth_inner_channel, 1, 1, 0, has_bn=True,has_relu=True, has_bias=False, norm_layer=norm_layer))
    ...
    def forward(self, data, label=None, aux_label=None):
        ...
        last_fm = None
        boder_out = []
        for i, (fm, pre_rrb, aft_rrb, head) in enumerate(zip(blocks,
                                                self.border_pre_rrbs,
                                                self.border_aft_rrbs,
                                                self.border_heads)):
            fm = pre_rrb(fm)
