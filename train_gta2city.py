...
def main():
    ...
    for epoch in range(args.EPOCH):
        ...
        if(loss_adv_target_value/(i+1) > best_adv_loss):
            best_adv_loss = loss_adv_target_value/(i+1)
            for i,batch in enumerate(tqdm(src_dataset.getValData())):
                src_img, labels = batch
                src_img = src_img.cuda()
                labels = labels.cuda()
                model.eval()
                ...
                if (i == 50):
                    break
            for i,batch in enumerate(tqdm(tar_dataset.getValData())):
